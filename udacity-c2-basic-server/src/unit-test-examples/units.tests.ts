...
describe('concat' ,() =>{
  ...
  it('should return Martin', () =>{
    const result = concat('Mar','tin');
    expect(result).to.equal('Martin');
  });

  it('should throw an error if one string is null', () => {
    expect (()=>{ concat('Mar','')}).to.throw('Empty string')
  });
